...
void QHexView::paintEvent(QPaintEvent *event) {
    ...
    for (int lineIdx = firstLineIdx, yPos = yPosStart; lineIdx < lastLineIdx; lineIdx += 1, yPos += m_charHeight) {
        ...
        for (int xPos = m_posHex, i = 0;
             i < m_bytesPerLine && ((lineIdx - firstLineIdx) * m_bytesPerLine + i) < data.size(); i++, xPos += 3 *
                                                                                                               m_charWidth) {
        ...
        for (int xPosAscii = m_posAscii, i = 0; ((lineIdx - firstLineIdx) * m_bytesPerLine + i) < data.size() &&
                                                (i < m_bytesPerLine); i++, xPosAscii += m_charWidth) {
...
std::size_t QHexView::cursorPos(const QPoint &position) {
    ...
    if (((std::size_t) position.x() >= m_posHex) &&
        ((std::size_t) position.x() < (m_posHex + (m_bytesPerLine * 3 - 1) * m_charWidth))) {
